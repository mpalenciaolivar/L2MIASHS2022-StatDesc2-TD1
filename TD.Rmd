...
```{r Ajustement-et-représentation-du-modèle}
model <- lm(Y ~ X)
plot(X, Y)
abline(model)
```
...
```{r Détails-de-calcul-des-coefficients-de-la-régression}
...
```{r Détail-du-calcul-des-résidus}
...
```{r Résidus-calculés-à-la-main}
Epsilon
```

Représentons maintenant les résidus :

```{r Représentation-des-résidus}
...
```{r chargement-donnees-et-modelisation}
